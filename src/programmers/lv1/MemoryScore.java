 ...
 * 진들의 추억 점수를 photo에 주어진 순서대로 배열에 담아 return하는 solution 함수를 완성해주세요.
 */
public class MemoryScore {
    public static int[] solution(String[] name, int[] yearning, String[][] photo) {
        int[] answer = new int[photo.length];
        HashMap<String, Integer> memoryScoreMap = new HashMap<>();

        int i = 0; //추억점수 인덱스
        for (String s : name) {
            memoryScoreMap.put(s, yearning[i++]);
        }

        int j = 0;//정답 인덱스
        for (String[] pictureOne : photo) {
            int memoryScore = 0;
            for (String people : pictureOne) {
                Integer score = memoryScoreMap.getOrDefault(people, 0);
                memoryScore += score;
            }
            answer[j++] = memoryScore;
        }
