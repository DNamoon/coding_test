 ...
 * 매개변수로 주어질 때, 경주가 끝났을 때 선수들의 이름을 1등부터 등수 순서대로
...
public class RunGame {
    ...
    public static String[] solution(String[] players, String[] callings) {
        ...
        for (String callName : callings) {
            Integer callNameRanking = rankingMap.get(callName);

            //맵에 담겨있는 순위도 바꿔주기
            rankingMap.put(callName, callNameRanking - 1);
            rankingMap.put(answer[callNameRanking - 1], callNameRanking);

            answer[callNameRanking] = answer[callNameRanking - 1];
            answer[callNameRanking - 1] = callName;
